...
# Try to load Tessera::Storage
print "\n2. Loading Tessera::Storage...\n";
eval {
    require Tessera::Storage;
    print "   ✓ Tessera::Storage loaded successfully\n";
};
if ($@) {
    print "   ✗ Failed to load Tessera::Storage: $@\n";
    exit 1;
}
...
my $storage;
eval {
    $storage = Tessera::Storage->new(config => $config);
