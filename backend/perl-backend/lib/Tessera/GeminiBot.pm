...
Tessera::GeminiBot - Google Gemini API Integration for Tessera

=head1 SYNOPSIS

    use Tessera::GeminiBot;
    
    my $bot = Tessera::GeminiBot->new(
        storage => $storage,
...
It integrates with Tessera's existing storage and knowledge graph systems to
