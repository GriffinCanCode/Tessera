...
sub _build_config {
    ...
    $config->{database}{path} //= 'data/wiki_knowledge.db';
...
sub _build_logger {
    ...
    my $log_file = $self->config->{logging}{file} || 'logs/wikicrawler.log';
    ...
    my $log_config = qq{
        log4perl.rootLogger = $log_level, FileAppender, ScreenAppender
        ...
        log4perl.appender.FileAppender.layout.ConversionPattern = [%d] %p %c - %m%n
...
sub _build_crawler {
    my $self = shift;
    return WikiCrawler::Crawler->new(config => $self->config);
}

sub _build_parser {
    my $self = shift;
    return WikiCrawler::Parser->new(config => $self->config);
}

sub _build_storage {
    my $self = shift;
    return WikiCrawler::Storage->new(config => $self->config);
}

sub _build_link_analyzer {
    my $self = shift;
    return WikiCrawler::LinkAnalyzer->new(config => $self->config);
}
...
WikiCrawler is a modern Perl-based system for crawling Wikipedia, following
...
WikiCrawler uses YAML configuration files. See config/crawler.yaml for options.
