...
pub fn main() !void {
    ...
    for (0..iterations) |_| {
        vector_ops.batch_cosine_similarity_zig(query, embeddings, num_embeddings, vector_dim, results);
    }
    ...
    for (0..iterations) |_| {
        total_filtered = @intCast(vector_ops.batch_similarity_with_threshold_zig(query, embeddings, num_embeddings, vector_dim, threshold, filtered_results, filtered_indices));
    }
