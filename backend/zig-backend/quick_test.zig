...
pub fn main() !void {
    ...
    const similarity = vector_ops.cosine_similarity_zig(&vec1, &vec2);
    ...
    const ortho_similarity = vector_ops.cosine_similarity_zig(&vec3, &vec4);
    ...
    const count = vector_ops.batch_similarity_with_threshold_zig(&query, &embeddings, 3, 2, 0.5, &filter_results, &filter_indices);
    ...
    // Normalize vectors
    vector_ops.normalize_vector_zig(perf_query);
    for (0..num_embeddings) |i| {
        const start = i * dim;
        const end = start + dim;
        vector_ops.normalize_vector_zig(perf_embeddings[start..end]);
    }

    // Time the operation
    var timer = try std.time.Timer.start();

    vector_ops.batch_cosine_similarity_zig(perf_query, perf_embeddings, num_embeddings, dim, perf_results);
