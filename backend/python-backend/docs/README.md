...
- **Perl**: Coordinates with existing Tessera system, handles Wikipedia processing and intelligent chunking
...
- Existing Tessera system

### 2. Automatic Setup

Run the setup script from the `backend/python-backend/` directory:
...
1. **Start the main Tessera API server** (if not already running):
   ```bash
   cd ../perl-backend/script/
   perl api_server.pl
   ```
...
The system integrates seamlessly with your existing Tessera:

### Perl Layer (`Tessera::GeminiBot`)
...
backend/perl-backend/lib/Tessera/
├── GeminiBot.pm          # Perl interface layer

backend/perl-backend/script/
...
2. Add corresponding methods to `Tessera::GeminiBot`
