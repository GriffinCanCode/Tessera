...
# Test that we can create WikiCrawler instance (needed by API)
my $config_file = "$FindBin::Bin/test_config.yaml";
$ENV{WIKICRAWLER_CONFIG} = $config_file;

require WikiCrawler;
my $wiki_crawler = WikiCrawler->new(config_file => $config_file);
isa_ok($wiki_crawler, 'WikiCrawler', 'WikiCrawler instance for API');
