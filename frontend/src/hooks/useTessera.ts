...
export function useSearchArticles(query: string, limit = 20) {
  ...
  return useQuery({
    ...
    queryFn: () => {
      ...
      return TesseraAPI.searchArticles(query, limit);
    },
    enabled: isEnabled,
...
export function useArticle(title: string) {
  return useQuery({
    ...
    queryFn: () => TesseraAPI.getArticle(title),
    enabled: !!title,
...
export function useKnowledgeGraph(options: GraphOptions = {}) {
  return useQuery({
    ...
    queryFn: () => TesseraAPI.buildGraph(options),
    staleTime: 5 * 60 * 1000, // 5 minutes
  });
}
...
export function useHealthCheck() {
  return useQuery({
    ...
    queryFn: TesseraAPI.healthCheck,
...
export function useCrawlMutation() {
  ...
  return useMutation({
    mutationFn: (request: CrawlRequest) => TesseraAPI.startCrawl(request),
    onSuccess: () => {
...
export function useExportMutation() {
  return useMutation({
    mutationFn: (options: ExportOptions) => TesseraAPI.exportGraph(options),
  });
}
...
export function useCleanupMutation() {
  ...
  return useMutation({
    mutationFn: (keepDays: number) => TesseraAPI.cleanup(keepDays),
    onSuccess: () => {
...
export function useKnowledgeHubs(limit = 10) {
  return useQuery({
    ...
    queryFn: () => TesseraAPI.getKnowledgeHubs(limit),
...
export function useRecentDiscoveries(limit = 10) {
  return useQuery({
    ...
    queryFn: () => TesseraAPI.getRecentDiscoveries(limit),
    refetchInterval: 30 * 1000, // Refresh every 30 seconds
